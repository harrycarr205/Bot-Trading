...
write route is protected by a same-origin check (CSRF guard) — not full
authentication, appropriate for this single-operator, localhost-only
tool, but note it still means an unauthenticated local client can
start/stop the live trading process, cancel orders, and edit risk
config, not just view data. Start the dashboard whenever you want to
look or manage the processes, stop it whenever you're done; the
