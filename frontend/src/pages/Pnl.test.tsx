...
function mockPnl(decision_ids: string[]) {
  vi.mocked(api.pnl).mockResolvedValue({
    ...
    realized: [{ id: "p1", ticker: "AAPL", pnl_amount: 250, closed_at: "2026-08-24T15:00:00", decision_ids }],
  });
...
describe("Pnl page", () => {
  ...
  it("cross-links a realized row to its ticker and its originating decision", async () => {
    ...
    // The first decision_ids entry — the entry decision of the closed round-trip.
    const closedCell = screen.getByRole("link", { name: /2026/ });
    expect(closedCell).toHaveAttribute("href", "/decisions/d1");
  });

  it("leaves the Closed cell as plain text when a row has no decision ids", async () => {
    mockPnl([]);
